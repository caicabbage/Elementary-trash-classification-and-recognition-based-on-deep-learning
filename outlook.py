...
class HelloFrame(wx.Frame):
    ...
    def __init__(self,*args,**kw):
        ...
        self.pnl = pnl
        st = wx.StaticText(pnl, label="垃圾识别：", pos=(200, 0))
        font = st.GetFont()
        font.PointSize += 10
        font = font.Bold()
        st.SetFont(font)
        ...
        self.SetStatusText("Garbage sorting is very important")
    ...
    def OnSelect(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            test_img = Image.open(dialog.GetPath())
            test_img = test_img.resize([150, 150])
            image = np.array(test_img)
            x = np.expand_dims(test_img, axis=0)
            y = model.predict(x)
            labels = {0: '纸板', 1: '玻璃', 2: '金属', 3: '纸张', 4: '塑料', 5: '垃圾'}
            predict = labels[np.argmax(y)]
            result_text = wx.StaticText(self.pnl, label=predict, pos=(320, 0))
...
if __name__ == '__main__':
    ...
    app = wx.App()
    frm = HelloFrame(None, title='Let us sorting garbage', size=(1000,600))
    frm.Show()
    app.MainLoop()